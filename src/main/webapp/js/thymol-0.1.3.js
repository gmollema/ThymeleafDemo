/*-------------------- Thymol - the flavour of Thymeleaf --------------------*
   ...
   Thymol version 0.1.3-SNAPSHOT Copyright 2012-2013 James J. Benson.
...
var thymol = function() {
	...
	var thHref = new ThObj("href");
	var thSrc = new ThObj("src");
	...
	function getChildren(base) {
		var thHrefSpecs = $(thHref.escp, base.thDoc);
		var thSrcSpecs = $(thSrc.escp, base.thDoc);
		var ths = $(thHrefSpecs).add(thSrcSpecs);
		ths.each(function() {		  
			var element = this;
			$(element.attributes).each(function() {
				var thAttr = this;
				if (thHref.name == thAttr.name || thSrc.name == thAttr.name) {
					processUrl(element, base, thAttr);
				}
			});		  
		});
		...
		ths = $(thIfSpecs).add(thUnlessSpecs).add(thSwitchSpecs);
		ths.each(function() {
			var element = this;
			$(element.attributes).each(function() {
				var thAttr = this;
	...
	function processUrl(element, base, thUrlAttr) {
		...
		if( !overridden ) {
		  var url = getURLPart(thUrlAttr.value);
		  if( url != "" ) {
		    element.removeAttribute(thUrlAttr.name);
		    if( thUrlAttr.name == thHref.name ) {
